...
class Star {
	...
	show() { //display particle
      if(random(0,1) <= 0.99){ //the stars twinkle by having a very small chance of not being drawn (1 in 400 chance every frame)
      push();
      noStroke();
      translate(this.x,this.y);
      scale(this.size);
...
class Asteroid {
	constructor(x, y, size, rotation, index) { //Asteroid class (x position, y position, size, rotation, place in list)
        ...
        this.index = index;
        this.speedx = random(speeds) * 1.2
        this.speedy = random(speeds) * 1.2
	}
	...
	update() {
      ...
      //this handles if the asteroid goes offscreen. Since it is in space, it wraps around the screen rather than bouncing off the edge of it.
      if(this.x > width + 25){
        this.x = 0
      }
      if(this.x < 0 - 25){
...
function setup() {
  ...
  for(starsMade = 0;starsMade < numOfStars; starsMade++){
    stars.push(new Star(random(0,width),random(0,height),random(0.9,1.1)))
              }       // xposition   y position    size
  for(shipsMade = 0;shipsMade < numOfShips; shipsMade++){
    ships.push(new Ship(random(0,width),random(0,height),random(0.9,1.1), ships.length))
              }       // xposition   y position    size              index
  for(asteroidsMade = 0;asteroidsMade < numOfAsteroids; asteroidsMade++){
    asteroids.push(new Asteroid(random(0,width),random(0,height),random(0.7,1.2), random(1,360), asteroids.length ))
...
function draw() {
  ...
  for(shipsShown = 0; shipsShown < ships.length - 1; shipsShown++){
    ...
    ships[shipsShown].update();
      ...
      for(let i = 0; i < ships.length; i++){
        if(isTouching(ships[shipsShown],ships[i]) && shipsShown != i && spawnDelay > timeBetweenSpawns){
          print("new ship!")
          ships.push(new Ship(random(0,width),random(0,height),random(0.9,1.1), ships.length))
  ...
  for(asteroidsShown = 0; asteroidsShown < asteroids.length ; asteroidsShown++){
   ...
   for(let i = 0; i < ships.length; i++){
     if(isTouching(ships[i], asteroids[asteroidsShown])){
       ships.splice(i,1)
     }
 }
}
